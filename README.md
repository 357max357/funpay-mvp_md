...
4. Я выдаю доступ к нужному плагину через `/grant`.
...
Бот запоминает, под каким аккаунтом FunPay ему работать. (в теории можно потом посмотреть и сделать мультиаккаунт за доп. плату)
...
### Где взять `tg_user_id` (тоже глянуть, может можно встроить команду для получения своего id в тг)
...
Это плагины, которые подключены в текущем коде и поддерживаются (там их больше, работают пока только те, что снизу есть):
...
# Поправка багов и доделки

1) Оки, тогда наверно нужно добавить, чтобы пользователь знал о доступе
2) Подключить autorent_steam и steamgifts_autolo + поправить с md форматом
